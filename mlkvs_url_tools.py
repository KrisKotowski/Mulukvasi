...
class URLTools:
    ...
    def get_url_content(self, a_url, a_headers='', a_cookies=''):
        try:
            gv.G_LOGGER.debug('start downloading URL {0}'.format(a_url))
            if (a_headers == '') and (a_cookies == ''):
                i_url_content = requests.get(a_url, headers=URLTools.C_HEADERS, timeout=URLTools.C_TIMEOUT)
            else:
                i_url_content = requests.get(a_url, headers=a_headers, cookies=a_cookies, timeout=URLTools.C_TIMEOUT)
